...
from augment import DataAugment, augment_tensorization
...
class Data(object):
    ...
    def reference_tensorization(self, path, augment=False):
        ...
        for filename in os.listdir(path)[:self.length]:
            ...
            img_tensor = (transformations(org_img.convert('L'))*255).type(torch.float)
            if augment == True:
                img_augment = DataAugment(img_tensor)
                img_tensor = img_augment.total_augment()
            ref_img_tensors.append(img_tensor)
        ref_img_tensors = torch.concat(ref_img_tensors, dim=0)[:,None,:,:]
    ...
    def dataloader(self, augment = False):
        ...
        #normalization
        # total_data = torch.concat([pos_data, neg_data], dim=0)
        # print(total_data.shape)
        # total_mean = torch.mean(total_data, dim=[0,2,3])
        # total_std = torch.std(total_data, dim=[0,2,3])
