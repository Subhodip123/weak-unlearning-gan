...
class MultirefBlockGAN(object):
    ...
    def output_perturbation(self, augment):
        ...
        plt.title("blocking results/class-hats vs tau")
        fig = plt.gcf()
...
#         plt.title("blocking results/class-bangs vs tau")
#         fig = plt.gcf()
#         fig.savefig(f'./results/class-bangs/multirun/multirefmlpblocking{len(z_pos)}.pdf')
#         #ROC curve
#         auc_score = round(auc(false_alarm_prob, detection_prob),3)
#         plt.figure(figsize=(10, 10))
#         plt.plot(
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig(f'./results/class-bangs/multirun/roc_mlp{len(z_pos)}.pdf')
#         return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
