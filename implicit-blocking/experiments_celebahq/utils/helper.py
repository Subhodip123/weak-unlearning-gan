...
def no_ref_class(net, gen_data, classno: int, classifier="None"):
    ...
    if len(gen_data) != 0:
        ...
        gen_dataset=DataLoader(gen_data,batch_size=64)
        gen_data_list=[]
        ...
        if(classifier=="Base Classifier"):
            ...
            # print(gen_data.shape)
                batches = transforms.Resize((218, 178))(batches).to(device)
                # print(gen_data.size())
                # pred_labels = self.classifier(gen_data).cpu().detach().numpy()
                score=net(batches).detach().cpu()
                batches.detach().cpu()
...
# def fid_density_coverage(input_tensor: torch.tensor, input_labels:torch.tensor, unblocked_tensor:torch.tensor, backbone='inception'):
#     """Calculates the density and coverage metrics from the given tensor"""
#     if len(unblocked_tensor) !=0:
#         actual_released_tensor = input_tensor[torch.where(input_labels==1)[0]][:len(unblocked_tensor)]
#         fake_released_tensor = unblocked_tensor
#         print(len(actual_released_tensor), len(fake_released_tensor))
#         if actual_released_tensor.size()[1] != 3:
#                actual_released_tensor = torch.concat([actual_released_tensor, actual_released_tensor, actual_released_tensor], dim=1)
#                fake_released_tensor = torch.concat([fake_released_tensor, fake_released_tensor, fake_released_tensor], dim=1) 
#         if backbone == 'resnet':
...
#             actual_released_tensor = transforms.Resize((224,224))(actual_released_tensor)
#             fake_released_tensor = transforms.Resize((224,224))(fake_released_tensor)
#             actual_released_tensor_data=DataLoader(actual_released_tensor,batch_size=64)
#             fake_released_tensor_data=DataLoader(fake_released_tensor,batch_size=64)
#             actual_features=[]
#             fake_features=[]
#             for batches in actual_released_tensor_data:
#                 actual_features.append(feature_extractor(batches).detach().cpu())

#             for batches in fake_released_tensor_data:
#                 fake_features.append(feature_extractor(batches).detach().cpu())
#             actual_features=torch.cat(actual_features,dim=0)
...
#                 actual_features.append(feature_extractor(batches))
#                 batches.detach().cpu()
#             for batches in fake_released_tensor_data:
#                 batches = batches.to(device)
#                 fake_features.append(feature_extractor(batches))
#                 batches.detach().cpu()
#             print(len(actual_features))
#             print(actual_features[0].shape)
#             print(len(fake_features))
#             actual_features=torch.cat(actual_features,dim=0)
#             fake_features=torch.cat(fake_features,dim=0)

#             # actual_fatures = feature_extractor(actual_released_tensor).detach().cpu()
#             # fake_features = feature_extractor(fake_released_tensor).detach().cpu()
#             density, coverage = compute_prdc(real_features=actual_features, fake_features=fake_features, nearest_k=3)
#     else:
#         density, coverage = 0,0
