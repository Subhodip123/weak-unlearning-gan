...
class TrainInverter(object):
    ...
    def evaluation(
        ...
        if model_best is None:
            ...
            model_best = torch.load("./results/inversion_xnorm.pt")
        model_best.eval()
        loss = 0.0
        N = 0.0
    ...
    def training(self):
        ...
        for epoch in range(self.training_epochs):
            ...
            if epoch == 0:
                torch.save(self.inverter, "./results/inversion_xnorm.pt")
...
def train_inverter(config=None):
    ...
    if torch.cuda.is_available():
        G = G.cuda()        
    # OPTIMIZER
    with wandb.init(config=config, project='optimized_cifar10_inverter'):
        config = wandb.config
        ...
        recon_orig(device=device, inverter_path="./results/inversion_xnorm.pt", generator=G, 
