...
class TrainInverter(object):
    def __init__(
        ...
        (
            self.train_loader,
            self.train_noise_loader,
            self.val_loader,
            self.val_noise_loader,
            self.test_data,
            self.test_noise
        ) = self.data_generator.train_val_test_split()
    ...
    def evaluation(
        ...
        if model_best is None:
            ...
            model_best = torch.load("./results/inversion_znorm.pt")
        model_best.eval()
        loss = 0.0
        N = 0.0
        for indx_batch, (data_batch, noise_batch) in enumerate(zip(data_loader, noise_loader)):
            data_batch = data_batch.to(device=device)
            noise_batch = noise_batch.to(device=device)
            loss_t = model_best.loss(data_batch, noise_batch)
    ...
    def training(self):
        ...
        for epoch in range(self.training_epochs):
            # TRAINING
            self.inverter.train()
            for indx_batch, (input_batch, actual_noise) in enumerate(zip(self.train_loader, self.train_noise_loader)):
                # data from training loader and stack the user ref image
                input_batch = input_batch.to(device)
                actual_noise = actual_noise.to(device)
                # loss and update
                loss = self.inverter.loss(input_batch, actual_noise)
                # print(loss)
            ...
            if epoch == 0:
                torch.save(self.inverter, "./results/inversion_znorm.pt")
...
def train_eval_inverter(config=None):
    ...
    # OPTIMIZE and train
    with wandb.init(config=config, project='optimized_mnist_inverterznorm'):
        config = wandb.config
