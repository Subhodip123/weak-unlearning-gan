...
class UserFeedback(object):
    ...
    def mine_neg_examples(self, pos_noise_tensor:torch.tensor, noise_tensor) -> torch.tensor:
        ...
        gen_neg_tensor = noise_tensor[torch.tensor(random_selected_locations)]
    ...
    def feedback_tensorization(self):
        ...
        noise_tensor = self.inverter.forward(gen_data)
        # print(gen_data.size())
        ...
        if self.pos_neg:
            ...
            pos_noise = noise_tensor[pred_refclass_loc]
            neg_noise = noise_tensor[pred_nonrefclass_loc]    
        else: 
            pred_refclass_loc = torch.where(pred_class_tensor==self.classno)[0][:self.length]
            pos_noise = noise_tensor[pred_refclass_loc]
            neg_noise = self.mine_neg_examples(pos_noise_tensor=pos_noise, noise_tensor=noise_tensor)
