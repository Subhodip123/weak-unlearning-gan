...
def no_ref_class(net, gen_data, classno: int):
    ...
    if len(gen_data) != 0:
        ...
        # print(gen_data.size())
        pred_labels = net(gen_data).cpu().detach().numpy()
...
def projection_similarity(z_target: torch.tensor, z_input: torch.tensor):
...
def mine_neg_examples(gen_tensor: torch.tensor, inverter_path:str, pos_user_ref:torch.tensor,
                       projection_sim) -> torch.tensor:
    """Given the inverter, generated tensor, and positive user"""
    inverter = torch.load(inverter_path)
    z_pos = inverter.forward(pos_user_ref)
    z_gen = inverter.forward(gen_tensor)
    z_pos_mean = torch.mean(z_pos, dim=0)
    pos_sim_arr = projection_sim(z_pos_mean, z_pos)
    generated_sim_arr = projection_sim(z_pos_mean, z_gen)
    ...
    gen_neg_tensor = gen_tensor[torch.tensor(random_selected_locations)]
