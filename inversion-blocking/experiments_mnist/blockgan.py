...
class MultirefBlockGAN(object):
    ...
    def __init__(self, inverter_path, user_feedback, test_tensors, classifier_net, classification_func, class_no) -> None:
    ...
    def output_perturbation(self, augment):
        ...
        fig = plt.gcf()
        fig.savefig(f'./results/class-5/multirun/multiref_blocking{len(z_pos)}.pdf')
        ...
        fig2 = plt.gcf()
        fig2.savefig(f'./results/class-5/multirun/roc_multiref{len(z_pos)}.pdf')
...
class MultirefSVMBlockGAN(object):
    ...
    def output_perturbation(self, augment):
        ...
        plt.title("blocking results vs tau")
        fig = plt.gcf()
        fig.savefig(f'./results/class-5/multirun/multirefsvmblocking{len(z_pos)}.pdf')
        #ROC curve
        auc_score = round(auc(false_alarm_prob, detection_prob),3)
        plt.figure(figsize=(10, 10))
        plt.plot(
        ...
        plt.legend([f"auc score={auc_score}"])
        fig2 = plt.gcf()
        fig2.savefig(f'./results/class-5/multirun/roc_svm{len(z_pos)}.pdf')
        return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
...
# class MultirefBlockGAN(object):
#     """blocks the output of the user_ref image"""
#     def __init__(self, inverter_path, pos_user_ref, neg_user_ref, test_tensors, classifier_net, classification_func,class_no) -> None:
#         self.inverter =  torch.load(inverter_path)
#         self.pos_ref_imgs = pos_user_ref.reference_tensorization().to(device)
...
#     def target_pos_neg_latent(self, aug=False):
#         """It returns the target class's representation"""
#         z_pos = self.inverter.forward(self.pos_ref_imgs)
...
#         plt.title("blocking results vs tau")
#         fig = plt.gcf()
#         fig.savefig(f'./results/class-8/multirun/multiref_blocking{len(self.pos_ref_imgs)}.pdf')
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig(f'./results/class-8/multirun/roc_multiref{len(self.pos_ref_imgs)}.pdf')
#         return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
...
#         plt.title("blocking results vs tau")
#         fig = plt.gcf()
#         fig.savefig(f'./results/class-8/multirun/multirefsvmblocking{len(self.pos_ref_imgs)}.pdf')
#         #ROC curve
#         auc_score = round(auc(false_alarm_prob, detection_prob),3)
#         plt.figure(figsize=(10, 10))
#         plt.plot(
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig(f'./results/class-8/multirun/roc_svm{len(self.pos_ref_imgs)}.pdf')
#         return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
...
#                  test_tensors, classifier_net, classification_func, class_no ) -> None:
#         self.inverter =  torch.load(inverter_path)
...
#         plt.title("blocking results vs tau")
#         fig = plt.gcf()
#         fig.savefig(f'./results/class-8/multirun/multirefmlpblocking{len(self.pos_ref_imgs)}.pdf')
#         #ROC curve
#         auc_score = round(auc(false_alarm_prob, detection_prob),3)
#         plt.figure(figsize=(10, 10))
#         plt.plot(
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig(f'./results/class-8/multirun/roc_mlp{len(self.pos_ref_imgs)}.pdf')
#         return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
    
    
# class EncoderBlockGAN(object):
#     """blocks the output of the user_ref image"""
#     def __init__(self, inverter_path, encoder_path, pos_user_ref, 
#                  neg_user_ref, test_tensors, classifier_net, classification_func, class_no) -> None:
#         self.inverter =  torch.load(inverter_path)
...
#     def target_pos_neg_latent(self, space = 'latent'):
#         """It returns the target class's representation"""
#         if space == 'latent':
#             z_pos = self.inverter.forward(self.pos_ref_imgs)
#             z_neg = self.inverter.forward(self.neg_ref_imgs)
#         else:
#             z_pos = self.pos_ref_imgs.reshape((self.pos_ref_imgs.shape[0], 
#                                                self.pos_ref_imgs.shape[2]*self.pos_ref_imgs.shape[3]))
#             z_neg = self.neg_ref_imgs.reshape((self.neg_ref_imgs.shape[0], 
#                                                self.neg_ref_imgs.shape[2]*self.neg_ref_imgs.shape[3]))
#         z_target = torch.mean(z_pos, dim=0) - torch.mean(z_neg, dim=0)
#         return z_target, z_pos, z_neg
    
#     def cosine_similarity(self, z_target: torch.tensor, z_input: torch.tensor):
#         """calculates the cosine similarity between the reference img and input"""
...
#         batch_size = z_input.size()[0]
#         for i in range(batch_size):
#             similarity_score = torch.dot(z_target, z_input[i, :]) / (torch.norm(z_target) * torch.norm(z_input[i,:])) 
#             # print(similarity_score, i)
#             similarity_arr[i] = similarity_score 
#         return similarity_arr

#     def output_perturbation(self, space='latent'):
#         """It will give purturbed output based on the learning"""
#         z_target, z_pos, z_neg = self.target_pos_neg_latent(space=space)
#         z_pos = self.encoder.forward(z_pos)
#         z_neg = self.encoder.forward(z_neg)
#         if space == 'latent':
#             z_test = self.inverter.forward(self.test_tensors)
#             z_test = self.encoder.forward(z_test)
#         else:
#             z_test = self.test_tensors.reshape((self.test_tensors.shape[0], 
#                                                 self.test_tensors.shape[2]*self.test_tensors.shape[3]))
#         pos_sim_arr = self.cosine_similarity(z_target=z_target, z_input=z_pos)
#         neg_sim_arr = self.cosine_similarity(z_target=z_target, z_input=z_neg)
#         test_similarity_arr = self.cosine_similarity(z_target=z_target, z_input=z_test)
#         # best_sim_score = max([torch.max(torch.abs(pos_sim_arr)), 
#         #                           torch.max(torch.abs(neg_sim_arr)),
#         #                           torch.max(torch.abs(test_similarity_arr))])
#         # pos_sim_arr = pos_sim_arr/ best_sim_score
...
#         tau_max = torch.max(test_similarity_arr).item()
#         tau_min = torch.min(test_similarity_arr).item()
#         threshold_tau = torch.min(pos_sim_arr)
#         # pos_tau_sum = torch.sum(pos_sim_arr).item()
#         # neg_tau_sum = torch.sum(neg_sim_arr).item()
#         # threshold_tau = (pos_tau_sum+neg_tau_sum)/(len(pos_sim_arr)+len(neg_sim_arr))
#         # step = (tau_max-tau_min) / 1000
...
#         plt.title("blocking results vs tau")
#         fig = plt.gcf()
#         fig.savefig(f'./results/class-8/multirun/encoder_blocking{len(self.pos_ref_imgs)}.pdf')
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig(f'./results/class-8/multirun/roc_encoder{len(self.pos_ref_imgs)}.pdf')
#         return threshold_blocked_imgs, threshold_unblocked_output, threshold_ref_blocked, threshold_ref_unblocked, auc_score
    


# class SinglerefBlockGAN(object):
#     """blocks the output of the user_ref image"""
#     def __init__(self, inverter_path, user_ref, test_tensors, classifier_net, classification_func, class_no) -> None:
#         self.inverter =  torch.load(inverter_path)
#         self.user_ref_imgs = user_ref.augmented_tensorization().to(device)
#         self.test_tensors = test_tensors
#         self.classifier_net = classifier_net
#         self.classification_func = classification_func
#         self.class_no = class_no

#     def projection_similarity(self, z_ref: torch.tensor, z_input: torch.tensor):
#         """calculates the cosine similarity between the reference img and input"""
...
#         batch_size = z_input.size()[0]
#         for i in range(batch_size):
#             similarity_score = torch.dot(z_ref, z_input[i, :]) / torch.norm(z_ref) 
#             # print(similarity_score, i)
#             similarity_arr[i] = similarity_score 
#         best_sim_score = torch.max(torch.abs(similarity_arr))
#         similarity_arr = similarity_arr / best_sim_score
...
#         if space == 'latent':
#             z_test = self.inverter.forward(self.test_tensors)
#             z_user = self.inverter.forward(self.user_ref_imgs)
#         else:
#             z_test = self.test_tensors.reshape((self.test_tensors.shape[0], 
#                                                 self.test_tensors.shape[2] * self.test_tensors.shape[3]))
#             z_user = self.user_ref_imgs.reshape((self.user_ref_imgs.shape[0], 
#                                                  self.user_ref_imgs.shape[2] * self.user_ref_imgs.shape[3]))
#         z_ref_img = z_user[0,:]
#         z_ref_aug = z_user[1:,:]
#         user_similarity_arr = self.projection_similarity(z_ref=z_ref_img, z_input=z_ref_aug)
#         test_similarity_arr = self.projection_similarity(z_ref=z_ref_img, z_input=z_test)
#         tau_max = torch.max(test_similarity_arr).item()
#         tau_min = torch.min(test_similarity_arr).item()
#         threshold_tau = torch.mean(user_similarity_arr).item()
#         print(tau_max)
#         print(tau_min)
#         print("Mean thereshold value=", threshold_tau)
#         # step = (tau_max-tau_min)/self.class_no00
...
#                  linewidth='2',
#                  color='b',)
#         # plt.axvline(x=user_tau_max, color='r')
#         plt.axvline(x=threshold_tau, color='r')
#         plt.legend([f"no of ref-class in blocked output={threshold_blocked_imgs}", 
#                     f"no of blocked outputs={threshold_ref_blocked}"])
...
#         plt.title("blocking results vs tau")
#         fig1 = plt.gcf()
#         fig1.savefig('./results/class-8/multirun/multirun/blocking.pdf')
#         #ROC-AUC curve
#         auc_score = round(auc(false_alarm_prob, detection_prob),3)
#         plt.figure(figsize=(10, 10))
#         plt.plot(
...
#         plt.legend([f"auc score={auc_score}"])
#         fig2 = plt.gcf()
#         fig2.savefig('./results/class-8/multirun/roc_singleref.pdf')
#         return threshold_blocked_imgs, len(threshold_unblocked_output), threshold_ref_blocked, threshold_ref_unblocked, auc_score