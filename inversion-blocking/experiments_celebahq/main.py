...
datagen = DataGenerator(checkpoiint=generator_checkpoint, no_samples=5000, output_size=256, latent_dim=512)
train_tensor,  test_tensor = datagen.train_test_split()
print('test tensor where?', test_tensor.is_cuda)
...
#-------------Load Inverter------------------#
inverter_path="/home/ece/Subhodip/Unlearning/UnlearnGAN/Unlearn-Blocking/inversion-blocking/experiments_celebahq/utils/e4e_ffhq_encode.pt"
...
def baseclassifier_eval(length, class_no: int, actual_labels: torch.tensor,aug=False):
    ...
    test_dataset=DataLoader(test_tensor,batch_size=10)
    classifier_labels=[]
    for data_batch in test_dataset:
        data_batch = transforms.Resize((224,224))(data_batch).to(device)
        classifier_labels.append(classifier.predict(data_batch).cpu().detach())
        data_batch.cpu().detach()
    ...
    auc_score = auc(classifier_labels, actual_labels).item()
...
def blocking_eval(user_pos_neg):
    ...
    for length in lengths:
        ...
        user_feedback = UserFeedback(data_tensor=train_tensor, inverter = inverter_net,
                                     classifier=org_classifiernet, classno=36, length=length,pos_neg=user_pos_neg)

        # latent_classifier_path = f'/home/ece/Subhodip/Unlearning/UnlearnGANUnlearn-Blocking/latentclassifier/mnist/results/latentclassifier{length}.pt'
        
        multirefblocker = MultirefBlockGAN(user_feedback=user_feedback,
                                           inverter=inverter_net, 
                                           test_tensors=test_tensor,
        ...
        print("acc precision recall f1 auc fid density coverage", acc_svm, prec_svm, recall_svm, f1_score_svm, auc_score_svm, 
